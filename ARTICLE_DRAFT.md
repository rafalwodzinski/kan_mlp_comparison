# Architektury KAN vs klasyczne MLP w analizie medycznych danych tabelarycznych: Rygorystyczny benchmark i ewaluacja statystyczna

## 1. Wstęp i Tło Teoretyczne
...
## 2. Metodologia i Reprodukcyjność
...
Jako metryki oceny wybrano uogólniony Współczynnik Korelacji Matthewsa (MCC), zoptymalizowany pod niezbalansowane klasy medyczne, oraz Obszar Pod Krzywą ROC (AUROC).

## 3. Makroanaliza Skuteczności Klasyfikacyjnej
...
## 4. Ewaluacja Statystyczna Wyników
...
W celu uniknięcia paradoksu p-value, zastosowano Skorelowany Test t-Studenta (Benavoli et al., 2017), dostosowany do ewaluacji walidacji krzyżowej poprzez modelowanie estymacji bayesowskich, wykorzystując Strefę Praktycznej Równoważności (ROPE) ustaloną na $\pm 1\%$ MCC. 
...
Wyniki bayesowskie dowodzą twardej, matematycznej przewagi modelu JacobiKAN na zbiorze Cardiotocography (**82.4% pewności**) oraz bezsprzecznej dominacji MLP na zbiorze Pima Diabetes (**85.5%**). Rysuje to portret architektur KAN jako modeli doskonałych analitycznie, lecz potencjalnie przeuczonych na naiwnych liniowo danych.

## 5. Studium Przypadku: Prawdziwa "Wyjaśnialność" KAN (White Box)
...
*Rycina: Zjawisko interpretowalności. Porównanie uczywalnych jednowymiarowych funkcji aktywacji (WavKAN) do klasycznego globalnego trendu liniowego SiLU (MLP) na krawędzi wejściowej.*

Wykres stanowi twardy dowód w rozumieniu tego, czego pod spodem nauczył się algorytm, bez konieczności odwoływania się do ciężkich, heurystycznych przybliżeń jak biblioteka SHAP. Oś Y dla każdej wykresowanej funkcji reprezentuje amplitudę sygnału rzutowaną na sumujący węzeł ukryty. Szara, kropkowana linia obrazuje tradycyjny nieliniowy trend (w tym wypadku aktywację SiLU opartą na bazowej, pojedynczej wadze liniowej charakterystycznej dla MLP). Gruba, niebieska linia reprezentuje finalną wiedzę sieci KAN.

**Interpretacja Kliniczna (MDVP:PPQ i MDVP:Flo):**
...
## 6. Ograniczenia i Konkluzje

Szeroko zakrojony, standaryzowany badawczo audyt wykazał fundamentalne korzyści i niedoskonałości koncepcji Kołmogorowa-Arnolda w dziedzinie Deep Learningu dla biomedycznych zbiorów strukturalnych:
